...
def seed_data():
    ...
    if Service.query.count() == 0:
        services = [
            Service(name='Classic Haircut', description='Professional haircut with styling', price=250, duration=30,
                    category='Hair'),
            Service(name='Premium Haircut', description='Premium haircut with beard trim', price=400, duration=45,
                    category='Hair'),
            Service(name='Hair Coloring', description='Full hair coloring service', price=800, duration=90,
                    category='Hair'),
            Service(name='Beard Grooming', description='Precision beard trimming and shaping', price=150, duration=20,
                    category='Grooming'),
            Service(name='Royal Shave', description='Traditional hot towel shave', price=200, duration=25,
                    category='Grooming'),
            Service(name='Facial Treatment', description='Deep cleansing facial with massage', price=500, duration=45,
                    category='Skincare'),
            Service(name='Hair Spa', description='Nourishing hair spa treatment', price=600, duration=60,
                    category='Hair'),
            Service(name='Complete Grooming', description='Full grooming package', price=1000, duration=90,
                    category='Grooming')
        ]
...
def queue():
    if request.method == 'POST':
        ...
        flash(f'✅ Successfully joined queue! Position: #{position} | Estimated wait: {predicted_wait} minutes',
              'success')
        return redirect(url_for('queue'))
...
def admin_dashboard():
    ...
    for b in bookings:
        print(
            f"   Booking: {b.id} - {b.user.name if b.user else 'Unknown'} - {b.booking_date} - {b.booking_time} - {b.status}")
...
def cleanup_duplicate_queue_entries():
    with app.app_context():
        users = db.session.query(Queue.user_id).filter_by(status='waiting').group_by(Queue.user_id).having(
            db.func.count(Queue.id) > 1).all()
